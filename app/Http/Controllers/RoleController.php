...
class RoleController extends Controller
{
    ...
    public function setPermission(Request $request)
    {
        ...
        if ($request->has('sale-types-index')) {
            $permission = Permission::firstOrCreate(['name' => 'sale-types-index']);
            if (!$role->hasPermissionTo('sale-types-index')) {
                $role->givePermissionTo($permission);
            }
        } else
            $role->revokePermissionTo('sale-types-index');

        if ($request->has('sale-types-add')) {
            $permission = Permission::firstOrCreate(['name' => 'sale-types-add']);
            if (!$role->hasPermissionTo('sale-types-add')) {
                $role->givePermissionTo($permission);
            }
        } else
            $role->revokePermissionTo('sale-types-add');

        if ($request->has('sale-types-edit')) {
            $permission = Permission::firstOrCreate(['name' => 'sale-types-edit']);
            if (!$role->hasPermissionTo('sale-types-edit')) {
                $role->givePermissionTo($permission);
            }
        } else
            $role->revokePermissionTo('sale-types-edit');

        if ($request->has('sale-types-delete')) {
            $permission = Permission::firstOrCreate(['name' => 'sale-types-delete']);
            if (!$role->hasPermissionTo('sale-types-delete')) {
                $role->givePermissionTo($permission);
            }
        } else
            $role->revokePermissionTo('sale-types-delete');
